...
public class WarpSwipeBackLayout extends FrameLayout {
     ...
     */
    private List<SwipeBackLayout.OnSwipeListener> mListeners;

    public WarpSwipeBackLayout(Context context) {
        this(context, null);
    }

    public WarpSwipeBackLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public WarpSwipeBackLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    private void init() {
        mHelper = ViewDragHelper.create(this, new WarpSwipeBackLayout.ViewDragCallback());
     ...
     */
    public void addSwipeListener(SwipeBackLayout.OnSwipeListener listener) {
        if (mListeners == null) {
     ...
     * @param listener
     */
    public void removeSwipeListener(SwipeBackLayout.OnSwipeListener listener) {
        if (mListeners == null) {
            return;
        }
    ...
    public void attachToFragment(SupportFragment swipeBackFragment, View view) {
        addView(view);
    ...
    class ViewDragCallback extends ViewDragHelper.Callback {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            ...
            if (dragEnable) {
                ...
                if (mListeners != null && !mListeners.isEmpty()) {
                    for (SwipeBackLayout.OnSwipeListener listener : mListeners) {
            ...
            if (mScrollPercent > 1) {
                if (mFragment != null) {
                    if (mPreFragment instanceof RxSupportFragment) {
                        ((RxSupportFragment) mPreFragment).setLocking(true);
                    }
                    if (!mFragment.isDetached()) {
                         mFragment.popForSwipeBack();
                    }
                    if (mPreFragment instanceof SupportFragment) {
                        ((RxSupportFragment) mPreFragment).setLocking(false);
                    }
                } else {
        ...
        public int getViewHorizontalDragRange(View child) {
            ...
            // SwipeBackActivity
            if (mActivity != null && ((RxSupportActivity) mActivity).swipeBackPriority()) {
                return 1;
            }
            return 0;
        }
